...
class EmailBuilder(object):
    ...
    def flush(self):
        ...
        if self.body:
            if self.bodymimemain == "text":
                ...
                if body:
                    ...
                    newmsg.set_content(
                        self.body, maintype=self.bodymimemain, subtype=self.bodymimesub
                    )
            else:
                newmsg.set_content(
                    self.body, maintype=self.bodymimemain, subtype=self.bodymimesub
        ...
        for att in self.attachments:
            ...
            mt, st = mime.split("/")
            newmsg.add_attachment(att[0], maintype=mt, subtype=st, filename=fn)
...
class PstExtractor(object):
    ...
    def getipath(self, params):
        ipath = met_join(self.target + ".export", params["ipath"].decode("UTF-8"))
        ...
        try:
            ...
            self.em.rclog(
                "getipath doc len %d [%s] ipath %s" % (len(doc), doc[:20], ipath)
            )
    ...
    def getnext(self, params):
        ...
        try:
            ...
            if self.proc.returncode == 0:
                ...
                self.em.rclog(
                    "getnext: subprocess returned code %d" % self.proc.returncode
                )
                return (False, "", "", rclexecm.RclExecM.eofnow)
