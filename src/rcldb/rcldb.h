...
public:
    TermMatchEntry() {}
    TermMatchEntry(const std::string& t, int f, int d)
        : term(t), wcf(f), docs(d) {}
    TermMatchEntry(const std::string& t)
        : term(t) {}
...
public:
    ...
    std::string prefix;
    // Terms generated by spelling proximity: used to inform the user (transmitted through
    // Highlightdata)
    std::vector<std::string> fromspelling;
};
...
public:
     ...
     */
    void setExistingFlags(const std::string& udi, unsigned int docid);
     ...
     */
    bool addOrUpdate(const std::string &udi, const std::string &parent_udi, Doc &doc);
    ...
    bool purgeFile(const std::string &udi, bool *existed = nullptr);
    /** Delete subdocs with an out of date sig. We do this to purge
        ...
        will be done */
    bool purgeOrphans(const std::string &udi);
    ...
    bool deleteStemDb(const std::string &lang);
    ...
    int termDocCnt(const std::string& term);
     ...
     */
    bool addQueryDb(const std::string &dir);
    /** Remove extra database. if dir == "", remove all. */
    bool rmQueryDb(const std::string &dir);
    ...
    */
    ...
    static bool testDbDir(const std::string &dir, bool *stripped = nullptr);
    ...
    */
    ...
    bool filenameWildExp(const std::string& exp, std::vector<std::string>& names, int max);
     ...
     */
    bool getDoc(const std::string &udi, const Doc& idxdoc, Doc &doc);
    ...
    bool termWalkNext(TermIter *, std::string &term);
    void termWalkClose(TermIter *);
    /** Test term existence */
    bool termExists(const std::string& term);
    /** Test if terms stem to different roots. */
    bool stemDiffers(const std::string& lang, const std::string& term, const std::string& base);
    ...
    bool udiTreeMarkExisting(const std::string& udi);
